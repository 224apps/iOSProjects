...
class SpringsAndTransitionsViewController: UIViewController {
    ...
    @IBAction func segmentControlOnSelected(_ sender: UISegmentedControl) {
        ...
        if sender.selectedSegmentIndex ==  1 {
            self.addTextFieldWithTransition()
        } else {
            self.removeTextFieldWithtransition()
        }
    }
    
    @IBAction func continueOnButtonPressed(_ sender: UIButton) {
        // TODO: Swap footer label
        swapViewsFromTransiton()
    }
    ...
    func showContinueButton(){
        UIView.transition(with: self.continueButton, duration: 0, options: [.transitionFlipFromTop], animations: {
            self.continueButton.isHidden = false
        }, completion: nil)
    }
    
    func addTextFieldWithTransition(){
        UIView.transition(with: self.textfieldContainer, duration: 1.0, options: [], animations: {
